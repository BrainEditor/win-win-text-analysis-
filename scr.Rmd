...
gja_topics_totals <- gja_topics_totals %>%
...
#plotting
gja_topics_plot <-  gja_topics_totals  %>% 
...
```
The chart above gives us a better understanding of the general focus of policy discussions in each region. But are there specific phrases, leaders, or key policies that are also often discussed in tandem with "win-win". Instead of looking to the keywords, what if we try and pull this information out from the headlines themselves? 

To get a sesne of this, I investigated a few key phrases and policies that are often mentioned in key policy documents on national security and foreign policy, as well as popular slogans and phrases. These are terms I thought merited more investigation based on my qualitative research. I'd be interested to see what unique terms came from the headlines themselves. It's a new tool to me, but perhaps some natural language processing could be used to find other unique phrases from headlines that merit more investigation. 

The result is a very wide table of headlines. The columns reflect wheter or not a headline includes a given phrase, taking on a value of 1 if that phrase is included and 0 otherwise. We'll count the 1s and 0s in each of these columns to find out how often each topic is mentioned. 

This also has a lot of copy pasted text. In the future I could improve this by creating a function that combines the ifelse and str_detect calls and takes on the chinese phrase as an argument. 
...
headline_binaries <- gongying_all %>% 
  mutate(belt_and_road = ifelse(str_detect(gongying_all$headline, 
                                           pattern = "一带一路") == TRUE,
                                1, 0), 
         community_of_common_destnity = 
           ifelse(str_detect(gongying_all$headline,
                             pattern = "人类命运共同体") == TRUE,  1, 0),
         financial_crisis =
           ifelse(str_detect(gongying_all$headline,
                             pattern = "金融危机") == TRUE,
                                1, 0), 
         ...
         int_coop_bri_forum =
           ifelse(str_detect(gongying_all$headline, 
                             pattern = "国际合作高峰论坛") == TRUE,
                                1, 0))
```
Next, we create a summary stable that shows how often unique terms were mentioned. We sum each of our columns and igore missing values. To lay things out more clearly, we change columns to rows below. 
  
```{r cleaning and summarizing unique term counts}  
...
headline_topic_summaries <- headline_binaries %>% 
  ...
  print()
#collapse into just two columns - topic and mentions 
headline_topic_summaries <- pivot_longer(headline_topic_summaries, everything(), names_to = "topics") %>% 
  rename("total_mentions" = value)
```

\newpage 

## Visualizing Unique Terms in Win-Win Headlines

We'll again used a bar chart to visualize how often specific phrases are included in headlines alongside "win-win."

Once again, Belt and Road dominates other topics. But it's notable that security is a relatively common phrase. Some of the specific policy initiatives or security forums we might expect to be highlighted (like the Shanghai Cooperation Organization), however, are only rarely mentioned.  Instead, economic forums and initiatives are much more often discussed - and most are associated with the Belt and Road Initiative. 

```{r visualizing unique key words}
#plot

#we have a bunch of factors - is there a better way than a bar chart?

headline_terms_plot <- ggplot(headline_topic_summaries, 
                              aes(x = reorder(topics, total_mentions),
                                  y = total_mentions, 
                                  fill = total_mentions)) + 
  geom_bar(stat = "identity") +
  coord_flip() +
