...
I'm trying to incorporate the themeing for @ChinaCharts. Here's an example of their style

```{r china charts style}


   scale_fill_economist() +
  theme_wsj() +
    theme(text = element_text(size = 7)) +
  ggtitle("Monthly Retail Consumption ($), China, 01/19-02/21") +
  scale_y_continuous() +
  theme(legend.position="top", plot.title = element_text(size = 9, face = "bold"), 
        axis.title.y = element_text(color = "#000001", size=7, 
                                    vjust=1.9, face = "bold"), 
        axis.text.x = element_text(color="#000001", 
                           size=6, angle=345, face = "bold")) + 
  guides(fill=guide_legend(title="")) +
  labs(caption = c("©ChinaCharts", "Source: NBS/Exchange Rate=Monthly Avg")) +
...
#theming
  theme_wsj() +
    theme(text = element_text(size = 7)) +
  ggtitle("'Win-Win' Increasingly Popular in State Media") +
...
ggplot((aes(x=year, y=total_mentions, color = tag_english))) + 
  ...
  theme_wsj() +
    theme(text = element_text(size = 7)) +
  ggtitle("How Has State Media's 'Win-Win' Rhetoric Changed Over Time?") +
  labs( subtitle = "More Focus on Belt and Road, Less on U.S.") +
  scale_y_continuous() +
...
cbind(bilat_africa_country_summary, english_country_keys) %>% 
    rename("tag_english" = english_country_keys) %>%
  select(tag, tag_english, total_mentions) 
...
cc_bilat_summary_plot <- bilat_africa_country_summary  %>% 
 ...
 theme_wsj() +
    theme(text = element_text(size = 7)) +
  ggtitle("Where Does China Search for 'Win-Win's?") +
  labs(subtitle = "Developed Countries & BRI Partners") +
  scale_y_continuous() +
  scale_color_economist() +
